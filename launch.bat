...
:: searchproxy.ps1 takes its port from GEMMA_SEARCH_PORT, so export it here.
:: The assignment further down in the file server block runs AFTER this start,
:: which would leave the proxy bound to its own 11435 default while every health
:: probe above and the file server's /search route used the configured port.
